...
def create_project(project: schemas.ProjectCreate, current_user=Depends(auth.get_current_user), db: Session = Depends(database.SessionLocal)):
    return crud.create_project(db, project, current_user.id)

@app.get("/projects", response_model=List[schemas.ProjectResponse])
def get_projects(current_user=Depends(auth.get_current_user), db: Session = Depends(database.SessionLocal)):
    return crud.get_user_projects(db, current_user.id)

# === FILES ===

@app.post("/projects/{project_id}/files", response_model=schemas.FileResponse)
def create_file(project_id: int, file: schemas.FileCreate, current_user=Depends(auth.get_current_user), db: Session = Depends(database.SessionLocal)):
...
def get_file(file_id: int, current_user=Depends(auth.get_current_user), db: Session = Depends(database.SessionLocal)):
...
async def websocket_endpoint(websocket: WebSocket, file_id: int, token: str = None, db: Session = Depends(database.SessionLocal)):
